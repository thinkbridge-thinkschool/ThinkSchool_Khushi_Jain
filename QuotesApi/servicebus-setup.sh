...
# Creates the namespace, topic and subscriptions the API publishes to, and
# prints the connection string. Standard tier because topics are not available
# on Basic, and Standard bills by the hour whether or not anything is sent --
# delete the group when you are done.

group=rg-quotes-servicebus
location=eastus
topic=quote-events

suffix=$(az account show --query id -o tsv | tr -d '-' | cut -c1-12)
namespace="sbquotes$suffix"
...
# Two deliveries before the dead-letter queue takes it, so an abandoned message
# is retried once rather than looping.
...
# Moderation takes QuoteCreated only, which is what makes the two subscriptions
# different rather than two copies of the same thing.
...
echo "Set it with: dotnet user-secrets set \"ServiceBus:ConnectionString\" \"<the line below>\" --project QuotesApi" >&2
