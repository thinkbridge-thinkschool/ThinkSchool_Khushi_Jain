...
@Component({
  ...
  template: `
    <section class="card">
      <h2>Reading through the interceptor chain</h2>
      <p class="hint">
        Auth header, retry with backoff on idempotent GETs, one-shot refresh-and-replay on a 401,
        and ProblemDetails mapped to a typed error before it reaches this page.
      </p>
    ...
    <section class="card">
      <h2>Force a failure</h2>
      <p class="hint">
        Each button provokes a different response from the list endpoint, to show what the chain
        does with it. All three are GETs, so all three are retried with backoff.
      </p>
