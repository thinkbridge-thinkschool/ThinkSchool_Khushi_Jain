...
// GET /api/quotes/{id} returns the Quote entity, which is the same shape POST
// answers with: no ownerActive, and isDeleted instead. Always false on a 200,
// since the repository filters deleted rows out, but it is part of the shape.
export type QuoteDetail = CreatedQuote;

// The body POST /api/quotes accepts. Matches CreateQuoteRequest in
// Contracts/QuoteRequests.cs, whose validation the two form pages mirror.
